(() => {
  ...
  function applyScrollFocus() {
    const meta = document.getElementById("tagging-scroll-focus");
    if (!meta) return false;
    const target = meta.dataset.target;
    if (!target) return false;
    const step = document.getElementById(`tagging-step-${target}`);
    const input = inputScrollContainer();
    if (!step || !input) return false;
    clampScroll(input, step.offsetTop - 8);
    return true;
  }
  ...
  function restoreScrollPosition() {
    ...
    requestAnimationFrame(() => {
      runFocusOrRestore();
      requestAnimationFrame(runFocusOrRestore);
    });
  }
