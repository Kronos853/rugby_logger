...
**Последнее обновление:** 2026-07-08 (tagging-htmx-input-col: partial swap левой колонки + таймлайна без full reload)
...
| `/matches/<id>/statistics` | Read-only статистика матча (счёт + метрики home/away) |
...
- Пульт разметки: липкий блок **«ТЕКУЩЕЕ СОБЫТИЕ»** (режим НОВОЕ/РЕДАКТИРОВАНИЕ, чипы Игрок→Действие→Результат, тайм); **scroll-focus** к следующему шагу после capture/update; **HTMX** partial swap левой колонки + таймлайна (toolbar без reload)
